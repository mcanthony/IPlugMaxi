...
VST2=${VST2//\VST_FOLDER = }/IPlugMaxi.vst

VST3=`echo | grep VST3_FOLDER ../../common.xcconfig`
VST3=${VST3//\VST3_FOLDER = }/IPlugMaxi.vst3

AU=`echo | grep AU_FOLDER ../../common.xcconfig`
AU=${AU//\AU_FOLDER = }/IPlugMaxi.component

APP=`echo | grep APP_FOLDER ../../common.xcconfig`
APP=${APP//\APP_FOLDER = }/IPlugMaxi.app

# Dev build folder
RTAS=`echo | grep RTAS_FOLDER ../../common.xcconfig`
RTAS=${RTAS//\RTAS_FOLDER = }/IPlugMaxi.dpm
RTAS_FINAL="/Library/Application Support/Digidesign/Plug-Ins/IPlugMaxi.dpm"

# Dev build folder
AAX=`echo | grep AAX_FOLDER ../../common.xcconfig`
AAX=${AAX//\AAX_FOLDER = }/IPlugMaxi.aaxplugin
AAX_FINAL="/Library/Application Support/Avid/Audio/Plug-Ins/IPlugMaxi.aaxplugin"

PKG='installer/build-mac/IPlugMaxi Installer.pkg'
PKG_US='installer/build-mac/IPlugMaxi Installer.unsigned.pkg'
...
xcodebuild -project IPlugMaxi.xcodeproj -xcconfig IPlugMaxi.xcconfig -target "All" -configuration Release 2> ./build-mac.log
...
echo "setting icons"
echo ""
setfileicon resources/IPlugMaxi.icns $AU
...
setfileicon resources/IPlugMaxi.icns "${AAX}"
...
sudo ashelper -f "${AAX_FINAL}/Contents/MacOS/IPlugMaxi" -l ../../../Certificates/aax.crt -k ../../../Certificates/aax.key -o "${AAX_FINAL}/Contents/MacOS/IPlugMaxi"
#---------------------------------------------------------------------------------------------------------
...
# echo ""
# codesign -f -s "3rd Party Mac Developer Application: ""${CERT_ID}" $APP --entitlements resources/IPlugMaxi.entitlements
#  
...
echo ""
packagesbuild installer/IPlugMaxi.pkgproj
...
#set installer icon
setfileicon resources/IPlugMaxi.icns "${PKG}"
...
then
  ...
  sudo rm -R -f installer/IPlugMaxi.dmg
fi
